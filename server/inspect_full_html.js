  ...
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      scripts.forEach((s, idx) => {
        ...
        if (text.includes('share') || text.includes('comment') || text.includes('view') || text.includes('894626316942483')) {
          scriptSnippets.push({
            idx,
            length: text.length,
    ...
    console.log(JSON.stringify(results.metaData, null, 2));
    
    console.log('=== SCRIPT SNIPPETS ===');
    console.log(JSON.stringify(results.scriptSnippets, null, 2));
